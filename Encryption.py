...
import os, blake3, hmac, ipaddress
...
def encrypt(data: bytes, key: bytes) -> bytes:
    ...
    cipher=Cipher(algorithms.AES(blake3.blake3(key + salt).digest(32)), modes.CTR(iv), backend=default_backend()).encryptor()
    _hash = blake3.blake3(data).digest(32)
...
def decrypt(encrypted: bytes, key: bytes) -> bytes:
    cipher=Cipher(algorithms.AES(blake3.blake3(key + encrypted[16:48]).digest(32)), modes.CTR(encrypted[:16]), backend=default_backend()).decryptor()
...
def encryptGCM(data: bytes, key: bytes) -> bytes:
    salt = os.urandom(32)
    iv = os.urandom(12)
    enc_key = blake3.blake3(key + salt).digest(32)
